...
The rounded mean of the steps per day is **`r as.character(round(pa1_mean, 2))`**, and the median is **`r pa1_median`** 
...
```
The interval that contains maximum average number of steps is **`r pa1_interval`**
...
```
Total number of missing values in the dataset is **`r pa1_missing`**
...
The mean of the steps per day, after filling NA as averages for particular interval, is **`r as.character(round(pa1_mean2,2))`**, and the median is **`r as.character(round(pa1_median2,2))`**

The mean is exactly the same after replacing the NA with mean (which should not be a surprise). The median changed a little, by **`r as.character(round(pa1_median2 - pa1_median, 4))`**
